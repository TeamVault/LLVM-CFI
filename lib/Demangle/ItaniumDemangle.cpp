...
static std::pair<std::string, std::string> itaniumDemangleCommon(const char *mangled_name,
                                                                 int &status) {
  ...
  demangle(mangled_name, mangled_name + len, db, status);
  ...
  db.names.back().first += db.names.back().second;
  return {db.names.back().first, db.functionName};
}

char *llvm::itaniumDemangle(const char *mangled_name, char *buf, size_t *n, int *status) {
  ...
  auto demangled_pair = itaniumDemangleCommon(mangled_name, internal_status);

  if (internal_status == success) {
    size_t sz = demangled_pair.first.size() + 1;
    if (sz > internal_size) {
      auto *newbuf = static_cast<char *>(std::realloc(buf, sz));
      if (newbuf == nullptr) {
    ...
    if (buf != nullptr) {
      std::memcpy(buf, demangled_pair.first.data(), sz - 1);
      buf[sz - 1] = char(0);
    }
  } else
    buf = nullptr;
...
std::pair<std::string, std::string> llvm::itaniumDemanglePair(llvm::StringRef mangled_name, int &status) {
  return itaniumDemangleCommon(mangled_name.str().c_str(), status);
};